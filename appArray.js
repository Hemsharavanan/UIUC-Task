...
function generateOutputFiles() {
  try {
    ...
    const formattedTrainingCounts = Object.entries(trainingCompletedCounts).map(
      ([training, count]) => ({ training, count })
    );
    fs.writeFileSync(
      "countOfPeopleCompletedTrainings.json",
      JSON.stringify(formattedTrainingCounts, null, 2)
    );
    ...
    const formattedPeopleCompletedTrainingsGivenYear = Object.entries(
      peopleCompletedTrainingsGivenYear
    ).map(([training, people]) => ({
      training,
      fiscalYear: 2024,
      people,
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(formattedPeopleCompletedTrainingsGivenYear, null, 2)
    );
    ...
    const formattedExpiredTrainings = Object.entries(
      peopleCompletedExpiredTrainings
    ).map(([name, expiredTrainings]) => ({
      name,
      expiredTrainings: expiredTrainings.map((training) => ({
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(formattedExpiredTrainings, null, 2)
    );
  } catch (error) {
